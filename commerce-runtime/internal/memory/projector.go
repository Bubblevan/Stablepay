...
func NewProjector(source EpisodeSource, store MemoryStore, options ...ProjectorOption) *Projector {
	p := &Projector{source: source, store: store, clock: func() time.Time { return time.Now().UTC() }, ttl: DefaultTTL}
...
func (p *Projector) ProjectEpisode(ctx context.Context, episodeID string) ([]MemoryMutation, error) {
	...
	for _, delivery := range deliveries {
		...
		capabilityValue := ensureStats(capabilityStats, merchant+"\x00"+capability, merchant, capability)
	...
	for _, event := range events {
		if event.Observation == "TOOL_ERROR" {
			...
			if merchant != "" {
				...
				ensureStats(capabilityStats, merchant+"\x00"+capability, merchant, capability).delta.MerchantErrorCount++
			}
		}
	}
...
func applySwitchAway(events []EventView, merchants map[string]*projectionStats, capabilities map[string]*projectionStats) {
	...
	for _, event := range events {
		...
		if currentMerchant != "" {
			...
			key := currentMerchant + "\x00" + currentCapability
			ensureStats(capabilities, key, currentMerchant, currentCapability).delta.SwitchAwayCount++
		}
