...
func EconomicIdentityKey(episodeID, merchantDID, capabilityID, quoteHash string, amountMinor int64, currency string) string {
	identity := economicIdentity{EpisodeID: episodeID, MerchantDID: merchantDID, CapabilityID: capabilityID, QuoteHash: quoteHash, AmountMinor: amountMinor, Currency: strings.ToUpper(strings.TrimSpace(currency))}
...
func (i PaymentIntent) Validate() error {
	if strings.TrimSpace(i.IntentID) == "" || strings.TrimSpace(i.EpisodeID) == "" || strings.TrimSpace(i.MerchantDID) == "" || strings.TrimSpace(i.CapabilityID) == "" || strings.TrimSpace(i.QuoteHash) == "" || strings.TrimSpace(i.RequesterDID) == "" {
		return ErrInvalidIntent
	}
	if i.AmountMinor <= 0 || i.BudgetReservation != i.AmountMinor || strings.TrimSpace(i.Currency) == "" || i.EpisodeVersion == 0 {
		return ErrInvalidIntent
	}
	if strings.TrimSpace(i.IdempotencyKey) == "" || strings.TrimSpace(i.EconomicKey) == "" || i.ExpiresAt.IsZero() || i.CreatedAt.IsZero() || i.UpdatedAt.IsZero() || i.UpdatedAt.Before(i.CreatedAt) || !i.ExpiresAt.After(i.CreatedAt) {
		return ErrInvalidIntent
	}
	if i.EconomicKey != EconomicIdentityKey(i.EpisodeID, i.MerchantDID, i.CapabilityID, i.QuoteHash, i.AmountMinor, i.Currency) {
		return ErrInvalidIntent
	}
