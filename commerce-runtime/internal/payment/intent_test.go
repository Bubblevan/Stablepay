...
func validIntent(now time.Time) PaymentIntent {
	return PaymentIntent{
		IntentID: "intent-1", EpisodeID: "episode-1", MerchantDID: "merchant-1", CapabilityID: "capability-1",
		...
		EconomicKey: EconomicIdentityKey("episode-1", "merchant-1", "capability-1", "sha256:quote", 300, "USDC"),
		ExpiresAt:   now.Add(time.Hour), Status: IntentCreated, CreatedAt: now, UpdatedAt: now,
	}
}
...
func TestPaymentIntentEconomicIdentityAndExpiry(t *testing.T) {
	...
	if intent.EconomicKey != EconomicIdentityKey("episode-1", "merchant-1", "capability-1", "sha256:quote", 300, "USDC") {
	...
	changedAmount := EconomicIdentityKey("episode-1", "merchant-1", "capability-1", "sha256:quote", 301, "USDC")
