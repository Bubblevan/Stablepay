...
func TestErrorClassificationAndSanitization(t *testing.T) {
	...
	if got := ClassifyError(ErrEntitlementPending); got != Retryable {
		t.Fatalf("pending entitlement class=%s", got)
	}
	if got := ErrorCode(ErrEntitlementPending); got != "ENTITLEMENT_PENDING" {
		t.Fatalf("pending entitlement code=%s", got)
	}
