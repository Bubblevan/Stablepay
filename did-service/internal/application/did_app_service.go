...
import (
	...
	"github.com/stablepay/did-service/internal/domain/entity"             // 领域实体包
	"github.com/stablepay/did-service/internal/domain/gateway"            // 仓储接口包
	"github.com/stablepay/did-service/internal/infrastructure/encryption" // 加密工具包
)
...
func (s *DIDAppService) VerifySignature(ctx context.Context, cmd *VerifySignatureCmd) (*VerifySignatureResult, error) {
	...
	// 7. 优先验证调用方提供的 canonical payload。Payment Service 已将
	// timestamp/nonce 放入业务 canonical payload，若再次拼接会改变钱包
	// 实际签过的字节。保留旧版 message+timestamp+nonce 作为兼容回退。
	valid := ed25519.Verify(pubKeyBytes, []byte(cmd.Message), sigBytes)
	if !valid {
		legacySignData := fmt.Sprintf("%s%s%s", cmd.Message, cmd.Timestamp, cmd.Nonce)
		valid = ed25519.Verify(pubKeyBytes, []byte(legacySignData), sigBytes)
	}
...
// 参数 timestampStr: RFC3339 或 Unix 秒级时间戳
// 返回: 是否在有效窗口内
func (s *DIDAppService) verifyTimestampWindow(timestampStr string) bool {
	timestamp, err := time.Parse(time.RFC3339, timestampStr)
	if err != nil {
		unixSeconds, parseErr := strconv.ParseInt(timestampStr, 10, 64)
		if parseErr != nil {
			return false
		}
