...
func (s *TransferCmdService) Execute(ctx context.Context, cmd *TransferCmd) (*TransferResult, error) {
	...
	if cmd.SignedTxBase64 == "" {
		...
		if err := txIn.UnmarshalBase64(cmd.SignedTxBase64); err == nil && len(txIn.Message.AccountKeys) > 0 {
...
func (s *TransferCmdService) validateTransactionFormat(base64Tx string) error {
	if err := s.solanaGateway.ValidatePartiallySignedTransaction(base64Tx); err != nil {
		return err
	}
...
func (s *TransferCmdService) validateFeePayer(base64Tx string) error {
	...
	if err := s.solanaGateway.ValidateFeePayer(base64Tx, expectedFeePayer); err != nil {
		return err
	}
